...
#include "tiledata_titlescreen.c"
#include "smart_sprite.c"
#include "game_states.h"

typedef struct test_structure_ {
  UINT8 x;
  UINT8 y;
} test_structure;

void create_test_structure(test_structure *test_structure_pointer, UINT8 x, UINT8 y) {
  test_structure_pointer->x = x;
  test_structure_pointer->y = y;
}
...
void main() {
  ...
  UINT8 tiles_t2[0xB4];
  UINT8 tiles_t3[0xB4];
  UINT8 scroll_position_x = 0;
  BOOLEAN resume = TRUE;
  test_structure popo[255];
